...
#show: ieee.with(
	...
	abstract: [
		Relational strctures, such as graphs and hypergraphs, provide a versatile framework for modeling entity interactions across multiple domains. 
...
In this work, a _hypergraph_ (or family of sets) is a pair $H = (V, cal(E))$ where $V$ is a finite set of elements called _vertices_ and $cal(E) subset.eq cal(P)(V)$ is any subset of the power set $cal(P)(V)$ of $V$. A _simplicial complex_ is a pair $S = (V, Sigma)$ satisfying the properties that (a) if $v in V$, then ${v} in Sigma$ and (b) if $tau subset sigma$ for any $sigma in Sigma$, then $tau in Sigma$. Denote by $S_p$ the set of $p$-simplices of $S$. An _oriented_ simplex, denoted by $[sigma]$, is a simplex $sigma in S$ with an ordering on its vertices. For simplicity, we will always assume to be induced by an ordering on $V$. The $p$-th _chain group_ $C_p (S, bb(R))$ of $S$ is the vector space over $bb(R)$ with basis ${[sigma] : sigma in S_p }$. 

Let $S$ be a simplicial complex with $p$-simplices $S_p$. A _weight function_ of $S$ is any positive function $w: S -> (0, infinity)$, i.e. any function that maps every simplex $sigma in S$ to a strictly positive weight. $S$ is called _unweighted_ is $w(sigma) = 1$ for all $sigma in S$.
The boundary $partial_p[sigma]$ of an oriented $p$-simplex $[sigma] in S$ is defined as the alternating sum of its oriented co-dimension 1 faces, which collectively for all $sigma in S^p$ define the $p$-th _boundary matrix_ $partial_p$:
$ 
partial_p [i,j] = cases(
...
The main issue with the map from @eq_trivial_map—aside from the potential size of $S$—is that its dynamics are less understood due to its lack of well-defined asymptotic behavior, as its not strictly a weight function in the sense above. 
Indeed, given any $p in bb(N)$, a weight function defines an inner product $angle.l [sigma], [sigma']angle.r_w$ on the chain group $C_p(S, bb(R))$ as follows:  
...
Denote by $partial_p^ast : C_(p-1) -> C_p$ the adjoint of $partial_p$ under these inner products. The $p$-th _combinatorial Laplacian_ $cal(L)_p : C_p (S, bb(R)) -> C_p (S, bb(R))$ is defined as: 
...
Clearly, the above expression is only well-defined if the corresponding weight function $w$ takes strictly positive values, preventing the use of characteristic functions akin to @eq_trivial_map. 
...
Thus, both singletons and pairwise interactions (edges) are assigned unit weight, whereas higher-order simplices have successively lower weights. Some of the properties of this weight function are _strict posivity_, _monotonicity with respect to the face poset_, and _normalized scale_: 
...
The choice of matrix and matrix function inevitably produces different featurizations, some of which have proven useful for learning purposes. Suppose $X$ represents a design matrix, $A$ an adjacency matrix a graph, and $M$ a positive semi-definite matrix. Some  exemplary matrix function parameterizations are given below: 
...
- $f(A) = (I - alpha A)^(-1)$ $thick <-> thick$ PageRank
...
#figure(
	...
	caption: [
		Hypergraphs, their weighted closures, diffusion curves, and signatures. The 'width' of each edge is inversely proportional to its hypergraph-induced weight, changing both the flow rate across edges and the signatures. 
	]
) <fig_diffusion_sig>
...
Paramount to the interoperability of our proposed framework with modern machine learning method is its scalability. While computation of @eq_diag_mf can be accomplished by standard eigen-decomposition algorithms (e.g. Cuppens divide-and-conquer), most modern featurization methods are limited to quadratic time complexity, and ideally they ought to work with sparse inputs. 
